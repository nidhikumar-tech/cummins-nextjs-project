...
export default function CNGCapacityPredictionChart() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      try {
        const response = await fetch(`/api/cng-capacity-predictions?state=${selectedState}`);
        const result = await response.json();

        if (result.success && Array.isArray(result.data)) {
          const cleanData = result.data.map(item => ({
            year: Number(item.year),
            // Updated to use _mmcf fields
            actual: item.actual_capacity_mmcf ? Number(item.actual_capacity_mmcf) : null,
            predicted: item.predicted_capacity_mmcf ? Number(item.predicted_capacity_mmcf) : null,
            min: item.min_predicted_capacity_mmcf ? Number(item.min_predicted_capacity_mmcf) : null,
            max: item.max_predicted_capacity_mmcf ? Number(item.max_predicted_capacity_mmcf) : null,
  ...
  const chartData = useMemo(() => {
    ...
    return {
      labels,
      datasets: [
        // 1. Min Bound (Bottom of the shadow)
        {
          label: 'Min Bound',
          data: minValues,
          borderColor: '#fca5a5', 
        ...
        // 2. Max Bound (Top of the shadow + Fill)
        {
          label: 'Max Bound',
          data: maxValues,
          borderColor: '#fca5a5', 
  ...
  const options = useMemo(() => ({
    ...
    plugins: {
      ...
      title: {
        display: true,
        text: `CNG Capacity Forecast - ${selectedState}`,
        align: 'start',
  ...
  return (
    <div style={{ width: '100%', height: '500px', padding: '24px', background: 'white', borderRadius: '12px', border: '1px solid #e2e8f0', boxShadow: '0 1px 2px 0 rgba(0,0,0,0.05)' }}>
      ...
      {/* Controls Header - Left Aligned */}
      <div style={{ marginBottom: '20px', display: 'flex', justifyContent: 'flex-start', alignItems: 'center', gap: '12px' }}>
        
        {/* Dropdown */}
        <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
          <label htmlFor="cng-state-select" style={{ fontWeight: '600', color: '#475569', fontSize: '14px' }}>State:</label>
          <select
            id="cng-state-select" 
            value={selectedState}
