...
export default function CNGProductionBarChart() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      try {
        const response = await fetch(`/api/cng-bar-graph?label=${encodeURIComponent('Production')}`);
        const result = await response.json();

        if (result.success && Array.isArray(result.data)) {
          console.log('Production Data:', result.data);
          const subLabels = [...new Set(result.data.map(row => row.Sub_Label))];
          console.log('Production Sub-Labels:', subLabels);
          console.log('Production Sub-Label names:', subLabels);
        ...
        console.error('Error fetching CNG production data:', err);
        setError('Failed to load CNG production data');
        setAllData([]);
      } finally {
        setLoading(false);
      }
  ...
  const chartData = useMemo(() => {
    ...
    // Define colors for production sub-labels (vibrant distinct colors)
    const colorMap = {
      'Dry Gas Production': '#2563eb',
      'Supplemental Natural Gas': '#facc15',
      'Renewable Natural Gas': '#10b981',
    };
